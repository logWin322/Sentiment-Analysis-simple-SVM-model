...
def preprocess_string(s, tag):
    if s[-1] == '\n':
        s = s[:-1]
    s = s.replace('\t', '')
    if "<review id=" not in s and "<review>" not in s and "<reviews>" not in s and "</review>" not in s and "</reviews>" not in s:
        s = s.replace('<', '&lt;')
        s = s.replace('>', '&gt;')
    if tag == CN:
        s.replace('<<', '《')
...
def cvt_to_npz(tag):
    ...
    np.savez(os.path.join(Dataset_Dir, Tag_Name[tag], "%s_train_.npz" % Tag_Name[tag]), trainset_target , *trainset_data)

    testset_data = testset_n + testset_p
    testset_target = np.array([0] * len(testset_n) + [1] * len(testset_p))
    np.savez(os.path.join(Dataset_Dir, Tag_Name[tag], "%s_test_.npz" % Tag_Name[tag]), testset_target , *testset_data)
...
def preprocess():    
    for lan in Languages:
        ...
        if (not os.path.exists(os.path.join(Dataset_Dir, Tag_Name[lan], "%s_train_.npz" % Tag_Name[lan]))) or (not os.path.exists(os.path.join(Dataset_Dir, Tag_Name[lan], "%s_test_.npz" % Tag_Name[lan]))):
